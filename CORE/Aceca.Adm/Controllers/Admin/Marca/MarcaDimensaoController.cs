...
{
    public class MarcaDimensaoController : Controller
    {
        #region variaveis

        private readonly ILogger<MarcaDimensaoController> _logger;
        ...
        public MarcaDimensaoController(ILogger<MarcaDimensaoController> logger, AppDbContext db, IWebHostEnvironment env, IConfiguration cfg)
        {
            _logger = logger;
            _db = db;
            _appEnvironment = env;
        ...
        {
            return View("~/Views/Admin/Marca/MarcaDimensao.cshtml");
        }
        ...
        [HttpPost]
        public async Task<IActionResult> Create(Models.MarcaDimensao model)
        {
            try
            {
                if (ModelState.IsValid)
                {
                    ...
                    var newModel = new Models.MarcaDimensao
                    {
                        Descricao = !string.IsNullOrEmpty(model.Descricao) ? model.Descricao : null,
                        Ativo = model.Ativo
                    };

                    _db.MarcaDimensao.Add(newModel);
                    _db.SaveChanges();
        ...
        [HttpPost]
        public async Task<IActionResult> Edit(Models.MarcaDimensao model)
        {
            try
            {
                if (ModelState.IsValid)
        ...
        public async Task<IActionResult> Delete(int id)
        {
            try
            {
                ...
                var model = await _db.MarcaDimensao.AsNoTracking().FirstOrDefaultAsync(x => x.Id == id);
                ...
                _db.MarcaDimensao.Remove(model);
                _db.SaveChanges();
