...
{
    public class MarcaFinalidadeController : Controller
    {
        #region variaveis

        private readonly ILogger<MarcaFinalidadeController> _logger;
        ...
        public MarcaFinalidadeController(ILogger<MarcaFinalidadeController> logger, AppDbContext db, IWebHostEnvironment env, IConfiguration cfg)
        {
            _logger = logger;
            _db = db;
            _appEnvironment = env;
        ...
        {
            return View("~/Views/Admin/Marca/MarcaFinalidade.cshtml");
        }
        #endregion
        ...
        [HttpPost]
        public async Task<IActionResult> Create(Models.MarcaFinalidade model)
        {
            try
            {
                if (ModelState.IsValid)
                {
                    ...
                    var newModel = new Models.MarcaFinalidade
                    {
                        Descricao = !string.IsNullOrEmpty(model.Descricao) ? model.Descricao : null,
                        Ativo = model.Ativo
                    };

                    _db.MarcaFinalidade.Add(newModel);
                    _db.SaveChanges();
        ...
        [HttpPost]
        public async Task<IActionResult> Edit(Models.MarcaFinalidade model)
        {
            try
            {
                if (ModelState.IsValid)
        ...
        public async Task<IActionResult> Delete(int id)
        {
            try
            {
                ...
                var model = await _db.MarcaFinalidade.AsNoTracking().FirstOrDefaultAsync(x => x.Id == id);
                ...
                _db.MarcaFinalidade.Remove(model);
                _db.SaveChanges();
