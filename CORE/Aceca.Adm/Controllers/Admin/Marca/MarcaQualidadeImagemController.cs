...
{
    public class MarcaQualidadeImagemController : Controller
    {
        #region variaveis

        private readonly ILogger<MarcaQualidadeImagemController> _logger;
        ...
        public MarcaQualidadeImagemController(ILogger<MarcaQualidadeImagemController> logger, AppDbContext db, IWebHostEnvironment env, IConfiguration cfg)
        {
            _logger = logger;
            _db = db;
            _appEnvironment = env;
        ...
        {
            return View("~/Views/Admin/Marca/MarcaQualidadeImagem.cshtml");
        }
        ...
        [HttpPost]
        public async Task<IActionResult> Create(Models.MarcaQualidadeImagem model)
        {
            try
            {
                if (ModelState.IsValid)
                {
                    ...
                    var newModel = new Models.MarcaQualidadeImagem
                    {
                        Sigla= !string.IsNullOrEmpty(model.Sigla) ? model.Sigla : null,

                        Descricao = !string.IsNullOrEmpty(model.Descricao) ? model.Descricao : null,
                        Ativo = model.Ativo
                    };

                    _db.MarcaQualidadeImagem.Add(newModel);
                    _db.SaveChanges();
        ...
        [HttpPost]
        public async Task<IActionResult> Edit(Models.MarcaQualidadeImagem model)
        {
            try
            {
                if (ModelState.IsValid)
        ...
        public async Task<IActionResult> Delete(int id)
        {
            try
            {
                ...
                var model = await _db.MarcaQualidadeImagem.AsNoTracking().FirstOrDefaultAsync(x => x.Id == id);
                ...
                _db.MarcaQualidadeImagem.Remove(model);
                _db.SaveChanges();
