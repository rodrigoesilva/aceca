...
let var_Nome = 'Agenda -> Imagem',
    var_Controller = '/AgendaImagem',
...
function fn_GridComplete(grid) {
    ...
    if (countRows > 0) {
        ...
        Swal.fire({
            title: 'SEM DADOS!!',
            icon: 'info',
            html: `N&atilde;o h&aacute; dados para serem carregados!!`,
            focusConfirm: false,
            confirmButtonText: `<i class="ri-check-double-line"></i>&nbsp;Ok!`,
            customClass: {
                confirmButton: 'btn btn-label-secondary waves-effect'
...
function fn_PopGetObj() {
    ...
    const objFormData = {
        Id: $('#hdId').val(),
        Imagem: $('.form-add-new-item .dt-line-01').val(),
