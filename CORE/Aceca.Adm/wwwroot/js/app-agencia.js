...
let var_Nome = 'Ag&ecirc;ncias',
    var_Controller = '/Agencia',
    ...
    if (varTbl_Obj.length) {
        ...
        varTbl_Data = varTbl_Obj.DataTable({
            ...
            columnDefs: [
                ...
                // COLUNA - CompExibicaoLiberado
                {
                    data: 'CompExibicaoLiberado',
                    targets: 4,
                    className: "text-center",
                    render: function (data, type, full, meta) {
                        ...
                        if (type === 'display') {
                            ...
                            statusClass = full.CompExibicaoLiberado ? 'bg-label-success' : 'bg-label-danger';

                            statusText = full.CompExibicaoLiberado ? 'Sim' : 'N&atilde;o';

                            statusLayout = '<span name="spStatus" data-status=' + full.CompExibicaoLiberado + ' class="badge rounded-pill ' + statusClass + '"> ' + statusText + '</span> ';
...
function fn_Pop(obj, action) {
    ...
    (popAddNewItem.querySelector('#hdId').value = (obj === null ? 0 : obj.Id)),
        ...
        // Pop Dados
        //console.log("fn_Pop varItems_Row !", obj.Observacao);
        (popAddNewItem.querySelector('.dt-line-01').value = (obj === null ? '' : obj.NomeFantasia)),
        (popAddNewItem.querySelector('.dt-line-02').value = (obj === null ? '' : obj.Observacao)),
        (popAddNewItem.querySelector('.dt-line-03').checked = (obj === null ? '' : obj.CompExibicaoLiberado)),
        (popAddNewItem.querySelector('.dt-line-04').checked = (obj === null ? false : obj.Status));
...
function fn_PopGetObj() {
    ...
    const objFormData = {
        Id: $('#hdId').val(),
        NomeFantasia: $('.form-add-new-item .dt-line-01').val(),
        Observacao: $('.form-add-new-item .dt-line-02').val(),
        CompExibicaoLiberado: $('.form-add-new-item .dt-line-03').is(':checked'),
