...
/* ── User table (espelho da tabela MySQL, para modo offline/demo) ── */
const USUARIOS = [
...
async function fazerLogin(e) {
  ...
  } catch { /* backend indisponível → usa tabela local */ }

  /* 2. Fallback local */
  if (!user) {
    const u = USUARIOS.find(x => x.email === email && x.senha === senha);
    if (u) user = { nome: u.nome, email: u.email, cargo: u.cargo };
  }
  ...
  if (user) {
    ...
    window.location.href = 'index.html?bem-vindo=' + encodeURIComponent(user.nome);
  } else {
